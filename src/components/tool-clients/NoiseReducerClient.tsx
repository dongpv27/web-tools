...
// FFmpeg afftdn noise reduction. `nr` is the strength in dB (10..40), higher
// removes more noise but may sound watery.
const STRENGTH_NR: Record<Strength, number> = { light: 12, medium: 20, strong: 30 };

export default function NoiseReducerClient() {
  ...
  const download = () => {
    ...
    a.href = downloadUrl;
    a.download = file.name.replace(/\.[^.]+$/, '') + '-denoised.' + outputFormat;
    a.click();
  };
        ...
        <div className="flex gap-2">
          {(['light','medium','strong'] as Strength[]).map((s) => (
            <button key={s} onClick={() => setStrength(s)}
              className={`px-4 py-2 text-sm rounded-lg ${strength === s ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
              {s[0].toUpperCase() + s.slice(1)} ({STRENGTH_NR[s]} dB)
            </button>
          ))}
        </div>
        <p className="text-xs text-gray-500 mt-1">Stronger settings remove more noise but may add a watery / underwater artifact. Try Medium first.</p>
      </div>

      <label className="flex items-center gap-2">
        <input type="checkbox" checked={highpass} onChange={(e) => setHighpass(e.target.checked)} className="w-4 h-4 text-blue-600 rounded" />
        <span className="text-sm text-gray-700">Apply 80 Hz high-pass filter (removes low rumble)</span>
      </label>
