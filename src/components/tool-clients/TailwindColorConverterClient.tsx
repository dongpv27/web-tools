...
export default function TailwindColorConverterClient() {
  ...
  const findClosest = () => {
    ...
    const matches: { name: string; shade: string; hex: string; className: string; distance: number }[] = [];
    ...
    setAllMatches(matches.slice(0, 10));

    if (matches.length > 0) {
      setClosestMatch(matches[0]);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Input Color</label>
      ...
      <div className="flex gap-2">
        <button onClick={findClosest} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">Find Tailwind Color</button>
      ...
      {closestMatch && (
        <div className="space-y-4">
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-xs text-green-600 mb-2">Closest Match</p>
            <div className="flex items-center gap-3">
              <div
              ...
              />
              <div>
                <p className="text-lg font-bold text-green-800">{closestMatch.className}</p>
                <p className="text-sm text-green-600 font-mono">{closestMatch.hex.toUpperCase()}</p>
              </div>
              <CopyButton text={closestMatch.className} />
            </div>
          </div>

          <div className="p-4 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-500 mb-3">Top 10 Matches</p>
            <div className="space-y-2">
              {allMatches.map((match, index) => (
                <div key={index} className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm font-mono text-gray-700 flex-1">{match.className}</span>
                  <span className="text-xs text-gray-500">{match.hex.toUpperCase()}</span>
                  <CopyButton text={match.className} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-500 mb-2">Usage Examples</p>
            <div className="space-y-1 font-mono text-xs">
              <p className="text-gray-600">&lt;div class="bg-{closestMatch.className}"&gt;</p>
              <p className="text-gray-600">&lt;div class="text-{closestMatch.className}"&gt;</p>
